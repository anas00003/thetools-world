...
function readImage(file){return new Promise((resolve,reject)=>{const img=new Image();img.onload=()=>resolve(img);img.onerror=reject;img.src=URL.createObjectURL(file);});}
...
function blobFrom(canvas,type,quality){return new Promise(resolve=>canvas.toBlob(resolve,type,quality));}
function size(bytes){return bytes>1024*1024?(bytes/1024/1024).toFixed(2)+" MB":Math.round(bytes/1024)+" KB";}
function resultLink(box,blob,name,img){const url=URL.createObjectURL(blob);box.innerHTML=\`<p><strong>Ready:</strong> \${size(blob.size)}</p><img alt="Processed preview"><p><a class="button primary" download="\${name}" href="\${url}">Download image</a></p>\`;box.querySelector("img").src=img||url;}
...
if(compressor){compressor.addEventListener("submit",async e=>{e.preventDefault();const box=$("[data-result]");const file=$("[data-file]",compressor).files[0];if(!file){box.textContent="Choose an image first.";return;}box.textContent="Compressing...";let quality=Number($("[data-quality]",compressor).value)/100;const maxWidth=Number($("[data-max-width]",compressor).value)||undefined;const target=Number($("[data-target]",compressor).value)||0;let blob=await processOne(file,{quality,maxWidth,type:"image/jpeg"});while(target&&blob.size/1024>target&&quality>.35){quality-=.08;blob=await processOne(file,{quality,maxWidth,type:"image/jpeg"});}resultLink(box,blob,file.name.replace(/\\.[^.]+$/,"")+"-compressed.jpg");});}
const resizer=$("[data-resizer]");
if(resizer){const width=$("[data-width]",resizer),height=$("[data-height]",resizer),aspect=$("[data-aspect]",resizer);let ratio=0;$("[data-file]",resizer).addEventListener("change",async e=>{if(e.target.files[0]){const img=await readImage(e.target.files[0]);ratio=img.naturalWidth/img.naturalHeight;}});width.addEventListener("input",()=>{if(aspect.checked&&ratio)height.value=Math.round(Number(width.value)/ratio);});height.addEventListener("input",()=>{if(aspect.checked&&ratio)width.value=Math.round(Number(height.value)*ratio);});resizer.addEventListener("submit",async e=>{e.preventDefault();const box=$("[data-result]");const file=$("[data-file]",resizer).files[0];if(!file){box.textContent="Choose an image first.";return;}box.textContent="Resizing...";const type=$("[data-format]",resizer).value;const blob=await processOne(file,{type,quality:.9,width:Number(width.value),height:Number(height.value)});resultLink(box,blob,file.name.replace(/\\.[^.]+$/,"")+"-resized."+ext(type));});}
const converter=$("[data-converter]");
if(converter){converter.addEventListener("submit",async e=>{e.preventDefault();const box=$("[data-result]");const file=$("[data-file]",converter).files[0];if(!file){box.textContent="Choose an image first.";return;}box.textContent="Converting...";const type=$("[data-format]",converter).value;const blob=await processOne(file,{type,quality:.9});resultLink(box,blob,file.name.replace(/\\.[^.]+$/,"")+"."+ext(type));});}
const bulk=$("[data-bulk]");
if(bulk){bulk.addEventListener("submit",async e=>{e.preventDefault();const box=$("[data-result]");const files=Array.from($("[data-file]",bulk).files);if(!files.length){box.textContent="Choose images first.";return;}box.textContent="Preparing downloads...";const quality=Number($("[data-quality]",bulk).value)/100,maxWidth=Number($("[data-max-width]",bulk).value)||undefined;const links=[];for(const file of files.slice(0,20)){const blob=await processOne(file,{type:"image/jpeg",quality,maxWidth});const url=URL.createObjectURL(blob);links.push(\`<a class="button secondary" download="\${file.name.replace(/\\.[^.]+$/,"")}-processed.jpg" href="\${url}">\${file.name} - \${size(blob.size)}</a>\`);}box.innerHTML=\`<p>Prepared \${links.length} download\${links.length===1?"":"s"}.</p><div class="download-list">\${links.join("")}</div>\`;});}`;
}
